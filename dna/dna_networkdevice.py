...
api_url     = '/api/v1/network-device'

def get_network_device(module, dna_url, token):
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            api_endpoint=dict(type='str', no_log=True)
        )
    )
    ...
    request_url = dna_url + api_url

    if api_endpoint != None:
        request_url += api_endpoint

    network_device = get_network_device(module, request_url, token)
